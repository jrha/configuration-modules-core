...
Readonly my $REPOS_DIR => "/etc/apt/sources.list.d";
Readonly my $REPOS_TEMPLATE => "spma/apt-source.tt";
...
ok(defined($cmp->generate_repos($REPOS_DIR, $repos, $REPOS_TEMPLATE)), "Basic repository correctly created");


my $fh = get_file("$REPOS_DIR/a_source.list");
ok(defined($fh), "Correct file opened");

my $name = $repos->[0]->{name};
like("$fh", qr{^\# Name: $name$}m, "Source got the correct name");

my $url = $repos->[0]->{protocols}->[0]->{url};
like("$fh", qr{^deb\s+$URL\s}m, "Primary source URL added");

$url = $repos->[0]->{protocols}->[1]->{url};
like("$fh", qr{^deb\s+$url\s}m, "Secondary source URL added");
...
#$fh = get_file("/etc/apt/sources.list.d/a_repo.list");
...
is($cmp->generate_repos($REPOS_DIR, $repos, "an invalid template name"), 0, "Errors on template rendering are detected");
is($cmp->{ERROR}, 1, "Errors on template rendering are reported");
$fh = get_file("/etc/apt/sources.list.d/$name.list");
...
#
#$fh = get_file("$REPOS_DIR/$name.list");
#
...
$fh = get_file("$REPOS_DIR/$name.list");
like("$fh", qr{^deb\s+http://$PROXY_HOST$}m,
...
$fh = get_file("$REPOS_DIR/$name.list");
like("$fh", qr{^deb\s+http://$PROXY_HOST:$PROXY_PORT$}m,
     "Port number is rendered with the reverse proxy");

$repos->[0]->{protocols}->[0]->{url} = $URL;

# Not supporting forward proxies yet, they need to go into a file in apt.conf.d
...
=item * Many hosts listed as reverse proxies lead to many baseurls in the apt repo
...
$fh = get_file("$REPOS_DIR/$repos->[0]->{name}.list");
like($fh, qr{^deb\s+http://$PROXY_HOST:$PROXY_PORT$}m ,"First reverse proxy correctly rendered");
like($fh, qr{^deb\s+http://another:$PROXY_PORT$}m, "Second reverse proxy correctly rendered");
...
#$fh = get_file("/etc/apt.lists.d/a_repo.list");
