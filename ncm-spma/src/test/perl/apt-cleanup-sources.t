...
outdated repositories in /etc/apt/sources.list.d/ are removed.
...
Readonly my $REPO_DIR => "target/test/clean.sources.list.d";

sub initialize_repos
{
    mkpath($REPO_DIR);
    open(FH, ">", "$REPO_DIR/repository.list");
    open(FH, ">", "$REPO_DIR/repository2.list");
}
...
ok(!-e "$REPO_DIR/repository2.list", "Unwanted repo scheduled for removal");
ok(-e "$REPO_DIR/repository.list", "Wanted file not marked for deletion");
...
if ($EUID) {
  ...
  diag("Trying to set immutable bit for root user to create unremovable files. ".
       "Run 'chattr -i $REPO_DIR/*list' to cleanup in case of failure during this test.");

  system("chattr +i $REPO_DIR/*list");
}
...
ok(-e "$REPO_DIR/repository2.list", "Unlisted repo is kept under userpkgs");
